...
export function EditVehicleForm({
  ...
  const onSubmit = async (values: VehicleFormValues) => {
    try {
      const result = await updateVehicle(vehicleId, values);
      if (!result.success) {
        setError("root", { message: result.error });
      }
      // On success, updateVehicle calls redirect(`/fleet/${vehicleId}`) server-side
    } catch {
            ...
            <div className="flex flex-col gap-6">
              ...
              <div className="flex flex-col gap-2 sm:flex-row-reverse">
                ...
                <Button
                  ...
                  asChild
                >
                  <Link href={`/fleet/${vehicleId}`}>Cancel</Link>
                </Button>
              </div>
